...
def main():
    with psycopg.connect("postgresql://postgres:example@localhost:5432/postgres") as conn:
        with conn.cursor() as cur:
            ...
            with open(DATA_DIR / 'categories.csv', 'r', encoding='utf-8') as f:
                ...
                with cur.copy("COPY categories (category_id, category) FROM STDIN") as copy:
            ...
            with open(DATA_DIR / 'products.csv', 'r', encoding='utf-8') as f:
                ...
                with cur.copy("COPY product_categories (product_id, category_id) FROM STDIN") as copy:
                    for product_category in tqdm(product_categories, desc="Product categories"):
            ...
            with open(DATA_DIR / 'ratings.csv', 'r', encoding='utf-8') as f:
                ...
                ratings = [(row["userId"], row["productId"], row["rating"], 
                        row["timestamp"], row["title"], row["comment"]) for row in reader if row["userId"] != ""] 
                with cur.copy("COPY ratings (user_id, product_id, rating, timestamp, title, comment) FROM STDIN") as copy:
